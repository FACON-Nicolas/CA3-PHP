...
@section('content')
    ...
    @foreach ($posts as $post)
        @if(!$post->draft || (isset(Auth::user()->id) && Auth::user()->id == $post->user_id))
            <div class="sm:grid grid-cols-2 gap-20 w-4/5 mx-auto py-15 border-b border-gray-200">
                ...
                <div>
                    ...
                    <span class="text-gray-500">
                        By <a href="{{route("profile", $post->user->id)}}"
                              class="font-bold italic text-gray-800">{{ $post->user->name }}</a>
                    ...
                    @if (isset(Auth::user()->id) && Auth::user()->id == $post->user_id)
                        @if($post->draft)
                            <span class="float-right">
                                <form method="post" action="{{route('publish',['blog'=>$post->slug])}}">
                                    @csrf
                                    <button class="text-green-500 pl-3">Publish</button>
                                </form>
                            </span>
                        @endif
                        <span class="float-right">
